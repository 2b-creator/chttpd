...
int get_userid(const char *token)
{
    ...
    const char *paramValues[] = {token};
    const int paramLength[] = {0};
    res = PQexecParams(conn, "SELECT UserId FROM Users WHERE UUID = $1;", 1, NULL, paramValues, paramLength, paramLength, 0);
    if (PQresultStatus(res) != PGRES_TUPLES_OK)
    {
        fprintf(stderr, "SELECT failed: %s", PQerrorMessage(conn));
        PQclear(res);
    ...
    char *useridRaw = PQgetvalue(res, 0, 0);
    int userid = 0;
